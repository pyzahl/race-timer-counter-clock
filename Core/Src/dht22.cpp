...
/// Then the data is transmitted in a pulse width modulation. After 50�s of a low signal the data bus is held high
/// for ~ 26�s for a '0' and ~ 70�s for a high
...
/// The timer is configured to tick every �s, which is a prescaler of 24 for the 24MHz MBED clock.
...
DHT22_ERROR DHT22::readData() {
  ...
  timerInitStructure.Prescaler = 24; //1�s per clock
  ...
  {
      bytes[i/8] <<= 1;
      bytes[i/8] |= ((data[i]-data[i-1]) > 100)?(1):(0);      //Saved length should be 50�s + 26�s for a 0 and 50�s + 70�s for a one
  }
